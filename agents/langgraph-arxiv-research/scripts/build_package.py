...
def get_package_name_and_version(pyproject_path: str) -> tuple[str, str]:
    ...
    """
    print("***Log build: Parse pyproject.toml to get the package name and version.")
...
def build_zip_sc(sc_dir: Path) -> None:
    ...
    print("***Log build: 1. Builds a source distribution using Poetry.")
    subprocess.run(["poetry", "build", f"--output={sc_dir.parent}", "--format=sdist"], check=True)
    print("***Log build: 2. Extract the build archive.")
    shutil.unpack_archive(sc_dir.with_suffix(".tar.gz"), sc_dir.parent)
    
    print("***Log build: 3. Normalizes file timestamps to fix ZIP timestamp issues.")
    for file in sc_dir.parent.rglob("*"):
        if file.is_file():
            file.touch()
    
    print("***Log build: 4. Create a ZIP archive of the source directory.")
